...
$(function(){
    ...
    $('.item>.i_help').click(function(){
        $(this).parent('.item').find('.i_desc').toggleClass('hide');
    });

    // Input Clear
    var iText = $('.item>.i_label').next('.i_input');
 ...
 * Pager
 */
function Pager(element_id, options){
    var defaults = {
        page : 1,                                     //현재 페이지
        all_row : 100,                                //전체 페이지수
        row : 10,                                     //출력 row
        page_col : 10,                                //한페이지에 출력할 페이지 갯수

        first_btn_text : '<',                        //이전 버튼 문구
        last_btn_text : '>'                          //다음 버튼 문구
    };
    ...
    //이벤트 설정
    if(options.callback != null) this.onClickCallback = options.callback;
    if(options.onClickCallback != null) this.onClickCallback = options.onClickCallback;
    
    //페이지 링크 함수 설정
    document.getElementById(this.element_id).linkClick = this.onClickCallback;
    
    //초기 html 링크 설정
    this.pageNav(options.page, options.all_row);
}
 ...
 * @return
 */
Pager.prototype.onClickCallback = function(page) {
 ...
 * @return
 */
Pager.prototype.link = function(page) {
    return 'javascript:document.getElementById(\''+this.element_id+'\').linkClick('+page+');'; 
};
 ...
 * @return
 */
Pager.prototype.onHtmlCallback = function(page, all_row) {
    var html = '';
    
    //first
    html += '<a href="'+this.link(1)+'" class="first_btn">'+this.first_btn_text+'</a>'+"\n";

    //페이지 링크
    for(var i = this.first_page; i <= this.last_page; i++)     {
        if(this.page == i)
    ...
    //last
    html += '<a href="'+this.link(this.all_page)+'" class="last_btn">'+this.last_btn_text+'</a>'+"\n";
 ...
 * @return
 */
Pager.prototype.pageNav = function(page, all_row) { 
    if(document.getElementById(this.element_id) == null) {
        alert('element가 없습니다.');
        return;
    }
     
    this.all_row = all_row==null?this.all_row:all_row;
    this.page = page==null?1:page;
    
    //전체 페이지
    this.all_page = Math.ceil(this.all_row / this.row);
    if(this.all_page < 1) this.all_page = 1;
        
    //현재 페이지
    if (this.page > this.all_page) this.page = this.all_page;
    
    //현재 페이지의 첫번째 row와 마지막 row
    this.last_row = this.row * this.page;
    this.first_row = this.last_row - this.row + 1;
    if(this.last_row > this.all_row) this.last_row = this.all_row;
    if(this.last_row < 1) this.last_row = 1;

    //시작페이지와 마지막 페이지
    /*
    if ((this.page % this.page_col) == 0) {
        this.first_page = this.page - (this.page_col-1);
        // 현 페이지가 10단위이면 시작 페이지는 현페이지에 -9를 한 값
        this.last_page = this.page;
        // 현 페이지가 10단위 이면 마지막 페이지를 현 페이지 번호로 함
    } else {
        this.first_page = (Math.floor(this.page / this.page_col) * this.page_col) + 1;
        // 시작페이지
        this.last_page = this.first_page + (this.page_col-1);
        // 출력될 마지막 페이지 구함

        if (this.last_page > this.all_page) {
            this.last_page = this.all_page;
            // 출력할 마지막 페이지가 전체 페이지보다 작으면 마지막 페이지를 전체 페이지로
        }
    }
    */
    
    var left_cnt = Math.floor(this.page_col/2);
    var right_cnt = this.page_col-left_cnt-1;

    if(this.page <= left_cnt) {
        this.first_page = 1;
        this.last_page = this.page_col;
        if(this.all_page < this.last_page) this.last_page = this.all_page;
    } else if((this.page + right_cnt) >= this.all_page) {
        this.first_page = this.all_page - this.page_col + 1;
        if(this.first_page <= 0) this.first_page = 1;
        this.last_page = this.all_page;
    } else {
        this.first_page = this.page - left_cnt;
        this.last_page = this.page + right_cnt;
    }
    
    //page html 설정
    var html = this.onHtmlCallback(page, all_row);
 ...
 * jui iframe 모달 오픈
 */
function modalShow(option, width, height, param) {
    if(parent.jui_modal_arr == null) {
        parent.jui_modal_arr = {
           length: 0,
           push: function push(id, element) {
               [].push.call(this, {id:id,element:element});
           },
           pop: function pop() {
               var data = [].pop.call(this);
               return data.element;
           },
           shift: function shift() {
               var data = [].shift.call(this);
               return data.element;
           },
           get: function get(id) {
               for(i = 0; i < this.length; i++) {
                   if(this[i].id == id) return this[i].element;
               }
    ...
    var v = parent.jui_modal_arr.get(modal_id);
    if(v != null) {
        alert('이미 등록된 모달 아이디 입니다.('+modal_id+')');
        return;
    }

    var jui_modal = parent.$('<iframe src="' + url + '" frameborder="0" style="border:0" scrolling="Yes"></iframe>').dialog({
        title: "",
        autoOpen: true,
        width: width,
        height: height,
        ...
        open: function( event, ui ) {
            ...
            if(param != null && param.open != null) {
                param.open();
            }
        },
        close: function( event, ui ) { 
            $('body').css('overflow', '');
            parent.jui_modal_arr.remove(modal_id);
            
            if(param != null && param.close != null) {
                param.close();
            }
        }
    }).width(width).height(height);
    
    jui_modal.parent().find('.ui-dialog-content').css('padding', '0');
    //jui_modal.parent().find('.ui-dialog-titlebar').hide();
    jui_modal.dialog("option", "position", "center");
    
    parent.jui_modal_arr.push(modal_id, {
        id:modal_id,
        width:width,
        height:height,
        url:url,
...
function modalHide(id) {
    var data = null;
    if(id == null || id == '') {
        data = parent.jui_modal_arr.last();
    } else {
        data = parent.jui_modal_arr.get(id);
    }
...
/**
 * jui iframe 모달창의 부모 윈도우
 */
function modalParent(id) {
    var data = null;
    if(id == null || id == "") {
        if(parent.jui_modal_arr == null || parent.jui_modal_arr.length <= 1) {
            return parent;
        }
        
        data = parent.jui_modal_arr[parent.jui_modal_arr.length-2];
        data = data.element;
    } else {
        data = parent.jui_modal_arr.get(id);
    }
    
    return data.modal.get(0).contentWindow;
}

/**
 * 리스트 로딩중 show
 */
function searchLodingShow() {
    //로딩중 화면 열기
    var width = 200;
    var height = 30;

    //documentElement = ff, ie / body = webket
    var top = (document.body.scrollTop==0?document.documentElement.scrollTop:document.body.scrollTop) + 100;
    //보이는 화면의 가운데
    var left = (document.body.clientWidth-width)/2;

    if($("#contentLoading").length == 0) {
        $('body').append('<div id="contentLoading"><div class="loding_search"><span></span></div></div>');

        $("#contentLoading").css('display', 'none');
        $("#contentLoading").css('position', 'absolute');
        $("#contentLoading").css('width', width);
        $("#contentLoading").css('height', height);
    }
 ...
 * 모달 로딩중 show
 */
function modalLodingShow() {
    jui_modal_loding = $('<div class="loding_modal"><span></span></div>').dialog({
        modal: true,closeOnEscape:false,resizable: false,
        overlay: {opacity: 0.5,background: "black"}
    });
    jui_modal_loding.parent().css('padding', '0');
    jui_modal_loding.parent().find('.ui-dialog-content').css('padding', '0');
    jui_modal_loding.parent().find('.ui-dialog-titlebar').hide();
}

/**
 * 모달 로딩중 hide
 */
function modalLodingHide() {
	jui_modal_loding.dialog('close');
}

var SearchParam = function (options) {
    var defaults = {
    };
...
SearchParam.prototype.set = function (key, value) {
    try {
        ...
        Ast.Cookies.remove(key);
    } catch (e) {
        Ast.Cookies.set(key, value, 365);
    }

};

SearchParam.prototype.get = function (key, defaultValue){
    var value = null;

    try {
        value = localStorage.getItem(key);
    } catch (e) {
        value = Ast.Cookies.get(key);
    }
...
SearchParam.prototype.isNULL = function (key){
    var value = null;
...
SearchParam.remove = function (key){
    this.set(key,'',-1);
...
$.format.locale({
    date: {
        ...
        longDateFormat: 'yyyy-MM-dd h:mm:ss SSS'
    },
    number: {
        format: '#,##0.0#',
        groupingSeparator: ',',
...
if(typeof chrome != "undefined" && chrome.webview != null &&
    chrome.webview.hostObjects != null &&
    chrome.webview.hostObjects.sync != null &&
    parent.chrome.webview.hostObjects.sync.jsCallbacks != null) {
    external = parent.chrome.webview.hostObjects.sync.jsCallbacks;
}